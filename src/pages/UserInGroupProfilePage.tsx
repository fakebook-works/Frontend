  ...
  return <main ref={pageRef} className={`profile-destination group-member-profile${tab === 'posts' ? ' profile-columns-scroll-active' : ''}`}>
    <section className="profile-cover-card">
      ...
      <div className="profile-destination-header">
        ...
        <div className="profile-destination-title"><h1>{profile.displayName}<VerifiedBadge verified={profile.isVerified} size={20} /></h1><button type="button" className="group-context-link" onClick={onBack}><Avatar name={group.name} src={group.avatarUrl} size={26} fallback="initials" />{t('memberOfGroup', { group: group.name })}</button></div>
