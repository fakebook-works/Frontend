    ...
    <div className="modal-backdrop" role="presentation" onClick={() => !busy && onClose()}>
      <div className="modal auth-flow-modal password-reset-modal" role="dialog" aria-modal="true" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="modal-body">
          ...
          {step === 'done' && <div className="security-form password-reset-complete"><p className="form-success">{t('passwordResetComplete')}</p><button type="button" className="btn-primary block" onClick={onClose}>{t('continueToLogin')}</button></div>}
        </div>
      </div>
    </div>
  )
