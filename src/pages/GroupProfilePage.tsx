  ...
  return <div className="modal-backdrop" role="presentation" onClick={() => !busy && onClose()}>
    <form className="modal group-profile-edit-modal" onSubmit={submit} onClick={(event) => event.stopPropagation()}>
      ...
      <div className="group-profile-edit-fields">
        <label><span>{t('groupName')}</span><input autoFocus value={name} maxLength={INPUT_LIMITS.groupName} onChange={(event) => setName(event.target.value)} /></label>
        <label><span>{t('groupDescription')}</span><textarea rows={5} value={bio} maxLength={INPUT_LIMITS.groupDescription} onChange={(event) => setBio(event.target.value)} /></label>
...
function GroupAboutCard({ group, locale, admin, compact = false, onUpdated }: { group: SocialGroup; locale: string; admin: boolean; compact?: boolean; onUpdated: (group: SocialGroup) => void }) {
  ...
  function descriptionEditor() {
    ...
    return <div className={`profile-about-inline-editor${editingAll ? ' editing-all' : ''}`}><textarea autoFocus rows={2} maxLength={INPUT_LIMITS.groupDescription} value={descriptionValue} onChange={(event) => setDescriptionValue(event.target.value)} aria-label={t('groupDescription')} spellCheck={false} data-gramm="false" data-gramm_editor="false" />{!editingAll && editorActions('inline')}</div>
  }
        ...
        <div className="self-profile-header-shell group-profile-hero-shell">
          ...
          <div className="profile-destination-header group-profile-identity-row">
            <div className={`self-profile-avatar-wrap no-story group-profile-avatar-shell${avatarEditor.target ? ' editing-avatar' : ''}`}>
              ...
              ><img src={avatarEditor.target.previewUrl} alt="" draggable={false} style={avatarEditor.imageStyle} onLoad={(event) => avatarEditor.onImageLoad(event.currentTarget)} /></div> : <Avatar name={group.name} src={group.avatarUrl} size={156} fallback="initials" />}
