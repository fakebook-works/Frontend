...
export const GatewayHomePage = memo(function GatewayHomePage({ profile = null, friends: providedFriends, refreshToken = 0, detailPostId = null, onDetailClose, onNavigate, onOpenReel, onMessage, onNewConversation, onConversation }: {
  ...
  return <>
    ...
    {detailPostId && <Suspense fallback={<div className="modal-backdrop content-modal-backdrop shared-detail-loading" role="presentation"><span className="spinner" /></div>}><ContentDetailOverlay
      ...
      onMessage={onMessage}
      onStoryCreated={applyCreatedStory}
      onOpenImage={(detailPost, media, _index, initialPlaybackTime) => onNavigate ? onNavigate(mediaOverlayHref(detailPost.id, media.id)) : setDetailPhotoViewer({ contentId: detailPost.id, mediaId: media.id, mediaUrl: media.url, initialPost: detailPost, initialPlaybackTime })}
      onOpenReel={onOpenReel ?? ((reel) => onNavigate?.(reelOverlayHref(reel.id)))}
    /></Suspense>}
    ...
    {open && <BodyPortal><div className="modal-backdrop home-composer-backdrop" role="presentation" onClick={closeComposer}>
      <form className={selectedFiles.length > 0 ? 'modal home-post-modal has-media' : 'modal home-post-modal'} role="dialog" aria-modal="true" aria-label={t('createPost')} onSubmit={submit} onClick={(event) => event.stopPropagation()}>
        ...
        <div className={selectedFiles.length > 0 ? 'home-post-modal-body has-media' : 'home-post-modal-body'}>
          ...
          }} onSelect={(event) => setMentionCaret(event.currentTarget.selectionStart ?? content.length)} placeholder={composerPlaceholder} rows={selectedFiles.length > 0 ? 1 : 6} /><MentionSuggestions text={content} people={friends} textareaRef={textareaRef} caretIndex={mentionCaret} onSelected={selectMention} onDismiss={() => setMentionCaret(-1)} />{selectedFiles.length > 0 && renderEmojiPicker(true)}</div>
