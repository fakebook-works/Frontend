...
import { socialApi, type ProfileRelationshipState, type SocialOwnedMedia, type SocialProfile } from '../api/social'
...
export function ProfilePage({ profile, loading, error, canEdit, viewerId, onEdit, onNavigate }: { profile: SocialProfile | null; loading: boolean; error: string | null; canEdit: boolean; viewerId: string; onEdit: () => void; onNavigate: (path: string) => void }) {
  ...
  const [photos, setPhotos] = useState<SocialOwnedMedia[]>([])
  ...
  const loadPhotos = useCallback(async (cursor: string | null = null, append = false) => {
    ...
    try {
      const page = await socialApi.getOwnedMedia(profile.id, 0, 60, cursor)
  ...
  return <main className="profile-destination">
    <section className="profile-cover-card">
      ...
      <div className="profile-destination-header">
        ...
        {canEdit ? <button type="button" className="btn-soft" onClick={onEdit}><Icon name="edit" size={17} />{t('editProfile')}</button> : <ProfileActions profile={profile} relationship={relationship} loading={relationshipLoading} busyAction={busyAction} onFriend={friendAction} onFollow={followAction} onBlock={blockAction} />}
      </div>
    ...
    <div className="profile-destination-grid">
      ...
      <section className="profile-post-list">
        ...
        {tab === 'photos' && <div className="card profile-tab-card"><h2>{t('photos')}</h2>{photosLoading && photos.length === 0 ? <div className="state-card"><span className="spinner" /></div> : photos.length === 0 ? <p className="muted">{photosUnavailable ? t('profileMediaLoadError') : t('photosEmpty')}</p> : <><div className="profile-photo-grid">{photos.map((media) => <a key={media.id} href={media.url} target="_blank" rel="noreferrer"><img src={media.url} alt="" loading="lazy" /></a>)}</div>{photosHaveMore && <button type="button" className="btn-soft load-more-result" disabled={photosLoading || !photoCursor} onClick={() => void loadPhotos(photoCursor, true)}>{photosLoading ? t('loadingMore') : t('seeMore')}</button>}</>}</div>}
      </section>
    </div>
  </main>
}

function ProfileActions({ profile, relationship, loading, busyAction, onFriend, onFollow, onBlock }: { profile: SocialProfile; relationship: ProfileRelationshipState; loading: boolean; busyAction: string | null; onFriend: (action: 'send' | 'cancel' | 'accept' | 'reject' | 'unfriend') => void; onFollow: () => void; onBlock: () => void }) {
  const { t } = useI18n()
