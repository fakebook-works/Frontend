...
function ReelCard({ reel, viewerId, onNavigate }: { reel: SocialContent; viewerId: string; onNavigate: (path: string) => void }) {
  ...
  return <article className="reel-card"><div className="reel-canvas">{media ? selectedRatio ? <div className="reel-card-media-frame" style={{ aspectRatio: String(selectedRatio) }}>{media.type === 1 ? <video src={media.url} controls preload="metadata" style={{ objectPosition }} /> : <img src={media.url} alt="" style={{ objectPosition }} />}</div> : media.type === 1 ? <video src={media.url} controls preload="metadata" /> : <img src={media.url} alt="" /> : <div className="reel-missing"><Icon name="video" size={64} /><span>{t('mediaUnavailable')}</span></div>}<div className="reel-overlay"><button type="button" onClick={() => reel.author && onNavigate(`/profile/${reel.author.id}`)}><Avatar name={reel.author?.displayName ?? t('fakebookUser')} src={reel.author?.avatarUrl} size={42} /></button><div><button type="button" className="post-author-name" onClick={() => reel.author && onNavigate(`/profile/${reel.author.id}`)}><strong>{reel.author?.displayName ?? t('fakebookUser')}<VerifiedBadge verified={reel.author?.isVerified} /></strong></button><p>{reel.content}</p></div></div></div><Suspense fallback={<div className="content-actions-skeleton" />}><ContentActions viewerId={viewerId} contentId={reel.id} variant="reel" onNavigate={onNavigate} /></Suspense></article>
}
